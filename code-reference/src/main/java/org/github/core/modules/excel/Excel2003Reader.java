...
public class Excel2003Reader implements HSSFListener {
	...
	private boolean isBlankRow=true;
	...
	public void processRecord(Record record) {
		...
		if (record instanceof MissingCellDummyRecord) {
			...
			if (curRow > startRow) {
				String title="";
				if(thisColumn < titleRowlist.size()){
					title=titleRowlist.get(thisColumn);
				}
				Object valueo = rowReader.getCol(sheetIndex, curRow, thisColumn, title, " ");
				setValue(title, valueo);
			} else if(curRow==titleRow)  {
	...
	private void setValue(String title, Object valueo) {
		if(StringUtils.isNotBlank(Objects.toString(valueo,""))){
			isBlankRow=false;
		}
		if (cls == null) {
