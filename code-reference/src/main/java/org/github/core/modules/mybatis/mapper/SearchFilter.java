		...
		for (Entry<String, Object> entry : searchParams.entrySet()) {
			...
			if(names[1].lastIndexOf("date")>-1){
				if (DateUtil.isDate(value + "")) {
					tmp.put(key, DateUtil.strToDate(value+""));
				}
			}else if(names[1].lastIndexOf("dats")>-1){
				if (DateUtil.isDate(value + " 00:00:00")) {
					tmp.put(key, DateUtil.strToDateLong(value+" 00:00:00"));
				}
			}else if(names[1].lastIndexOf("datd")>-1){
				if (DateUtil.isDate(value + " 23:59:59")) {
					tmp.put(key, DateUtil.strToDateLong(value+" 23:59:59"));
				}
			}else{
				tmp.put(key,value);
			}
