...
import org.github.core.modules.mybatis.util.MapperUtil;
...
public class PagePlugin implements Interceptor {
		...
		if (invocation.getTarget() instanceof StatementHandler) {
			...
			try{
				...
				if (paramObject instanceof AbstractPageEntity) {
					...
					throw new Exception("分页dao方法必须要有入");
				}
				String sql = boundSql.getSql();
				String changedsql = changeSql(sql);
				Configuration configuration = (Configuration) metaStatementHandler.getValue("delegate.configuration");
				ParameterMapping.Builder builder = new ParameterMapping.Builder(configuration, "endrow", new IntegerTypeHandler());
				...
				if(!findStartAndEndrow(paramMappings)){	//存在就不要在加start和count参数
	...
	/**
	 * 找参数中是否存在start和endrow参数
	 * @param paramMappings
	 * @return
	 */
	private boolean findStartAndEndrow(List<ParameterMapping> paramMappings){
		if(paramMappings!=null && !paramMappings.isEmpty()){
			...
			for(ParameterMapping parameterMapping : paramMappings){
				...
				if("endrow".equals(parameterMapping.getProperty())){
					isEndrow = true;
				}
			}
			if(isStart && isEndrow){
