...
public class DateUtil {
    ...
    private static String[]
            M_N_MONTH = {"JAN", "FEB", "MAR", "APR", "MAY", "JUN", "JUL", "AUG", "SEP", "OCT", "NOV", "DEC"};

    public static void main(String[] args) {
        // System.out.println( DateUtil.formatToStr(new Date(), "yyyy-M-d"));
    ...
    public static boolean getSxsjIsOk(String sxsj) {
        ...
        String[] s = sxsj.split(",");
        String nt = DateUtil.getUserDate("HH:mm");
        for (String o : s) {
            String[] o1 = o.split("-");
            if (o1.length == 2) {
    ...
    public static int getAge(String birthday) {
        ...
        Date csrq = DateUtil.strToDate(birthday);
    ...
    public static List<Date[]> dateExclude(Date fromDate, Date toDate, String str) {
        ...
        int pre = DateUtil.getTwoDay(toDate, fromDate);
        for (int i = 0; i <= pre; i++) {
            dset.add(DateUtil.getPreDay(fromDate, i));
        }
        ...
        for (String s : strsz) {
            ...
            if (ssz.length > 0) {
                d1 = DateUtil.strToDate(ssz[0]);
            }
            if (ssz.length > 1) {
                d2 = DateUtil.strToDate(ssz[1]);
            }
            if (d1 == null && d2 == null) {
                continue;
            }
            ...
            int p = DateUtil.getTwoDay(d2, d1);

            for (int i = 0; i <= p; i++) {
                dset.remove(DateUtil.getPreDay(d1, i));
            }
        }
        ...
        while (iter.hasNext()) {
            ...
            if (da[0] == null) {
                ...
                if (dd.equals(DateUtil.getPreDay(da[1], 1))) {
                    da[1] = dd;
                } else {
                    list.add(da);
                    da = new Date[2];
    ...
    public static String Ftmwk(String tm) {
        ...
        if (c > 0) {
            Date d = DateUtil.formatToDate(tm, "HH:mm");
            tm = DateUtil.formatToStr(d, "HH:mm");
        }
        if (tm == null) {
            return "00:00";
        }
    ...
    public static String dateCommandTime(String date) {
        ...
        if (date == null || date.length() != 10 || !DateUtil.isRightDate(date, "yyyy-MM-dd")) {
            return null;
        }
        ...
        if (StringUtils.isNotBlank(nyear)) {
            ...
            // 该类的最上面
            String EnMonth = M_N_MONTH[Integer.parseInt(nmonth) - 1];
